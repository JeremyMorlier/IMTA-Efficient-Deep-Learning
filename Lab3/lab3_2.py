...
def pruning(model, pruning_threshold, prune_method, first_load, cpt=None) :
    if first_load :
    ...
    # Apply pruning
    prune.global_unstructured(parameters_to_prune, pruning_method=prune_method, amount=pruning_threshold,)

# Only Works for Conv2D in Densenet121
def get_weights_stat(model) :
    num_nonzero = 0
    num_param = 0
...
# Pruning
pruning(model, pruning_rate, prune.L1Unstructured, True, loaded_cpt)
stats.append(get_weights_stat(model))
