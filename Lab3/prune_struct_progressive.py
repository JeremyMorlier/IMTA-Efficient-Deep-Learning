...
def pruning(model, pruning_threshold, first_load, cpt=None) :
    if first_load :
        model.load_state_dict(cpt)
...
pruning_list = [0.05, 0.05, 0.05, 0.05 ]
result_accuracies = []
result_loss = []
result_num_nonzero = []
result_num_param = []
...
for element in pruning_list :
    num_nonzero = 0
    num_param = 0
    pruning(model, element, False)
    for name, layer in model.named_modules() :
        if isinstance(layer, torch.nn.Conv2d) :
            temp = name.split(sep='.')
            if (len(temp) == 3 ) :
...
plt.grid()
plt.title("Progressive pruning (pruning_rate=0.05)")
plt.xlabel(" Weights removed %")
plt.ylabel("Accuracy")
plt.savefig("prune_struct_progressive.png")
plt.show()

fig = plt.figure()
plt.plot(pruning_list, result_accuracies)
plt.grid()
plt.title("Progressive pruning (pruning_rate=0.05)")
plt.xlabel(" L1 norm threshold")
plt.ylabel("Accuracy")
plt.savefig("prune_struct2_progressive.png")
plt.show()

fig = plt.figure()
plt.plot((result_num_param - result_num_nonzero)/result_num_param, pruning_list)
plt.grid()
plt.title("Pruning")
plt.xlabel(" L1 norm threshold")
plt.ylabel("pruning")
plt.savefig("prune_struct3_progressive.png")
plt.show()