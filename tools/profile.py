...
def count_conv2d(m, x, y):
    ...
    for kernel in range(m.weight) :

        if (torch.sum(torch.Tensor([kernel.numel()]))!=0 ) : 
            #ops += (kernel_mul + kernel_add)/m.groups + bias_ops
            ops += (kernel_mul + kernel_add)/m.groups
...
def profile(model, input_size, custom_ops = {}):
    ...
    def add_hooks(m):
        ...
        for p in m.parameters():
            ...
            m.total_params -= torch.sum(torch.Tensor([p.numel()]) == 0 ) / quant_factor
