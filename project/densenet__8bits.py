...
def run_epoch(loader, batchsize, model, criterion, optimizer, scheduler=None, mode="train"):
    ...
    with torch.set_grad_enabled(mode == "train"):
        for i, (inputs, labels) in enumerate(loader):
            ...
            optimizer.zero_grad()
            if(model.conv1.weight.dtype == torch.half) :
...
# Expérience 1
experimentName = "DenseNet_Lab2_1"
# Description
...
results_no_quant = run_epoch(test_loader, batchSizeTest, model, criterion, optimizer, scheduler, mode="eval")

quantized_model = torch.quantization.quantize_dynamic(model, {torch.nn.Linear}, dtype=torch.qint8)
results_quant = run_epoch(test_loader, batchSizeTest, quantized_model, criterion, optimizer, scheduler, mode="eval")

print(results_no_quant)
print(results_quant)